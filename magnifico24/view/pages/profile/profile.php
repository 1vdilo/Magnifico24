									...
									<div class="order-details hidden">
										<table>
											...
											<tbody>
												...
												?>
													<tr>
														<td data-label="Товар"><?= $detal['title'] ?></td>
														<td data-label="Материал"><?= GetTitle::titleMaterials($detal['materialsID']) ?></td>

														<?php if ($detal['colourIMG'] == null): ?>
															<td data-label="Цвет">Нет</td>
														<?php else: ?>
															<td data-label="Цвет"><?= GetTitle::titleColour($detal['colourIMG']) ?></td>
														<?php endif; ?>

														<?php if ($detal['colourIMG'] == null): ?>
															<td data-label="Цвет (картинка)">Нет</td>
														<?php else: ?>
															<td data-label="Цвет (картинка)"><img width="60" height="20" src="<?= $detal['colourIMG'] ?>" alt=""></td>
														<?php endif; ?>

														<?php if ($detal['topMaterials'] == null): ?>
															<td data-label="Верхний материал">Нет</td>
														<?php else: ?>
															<td data-label="Верхний материал"><?= $detal['topMaterials'] ?></td>
														<?php endif; ?>

														<?php if ($detal['topColourIMG'] == null): ?>
															<td data-label="Цвет верха (картинка)">Нет</td>
														<?php else: ?>
															<td data-label="Цвет верха (картинка)"><img width="60" height="20" src="<?= $detal['topColourIMG'] ?>" alt=""></td>
														<?php endif; ?>

														<td data-label="Количество"><?= $detal['quantity'] ?></td>
														<td data-label="Сумма"><?= $detal['price'] * $detal['quantity'] ?>$</td>
													</tr>
												<? endforeach ?>
											</tbody>
